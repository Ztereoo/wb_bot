import json
import telebot
from telebot import types

from config import TOKEN

import time
import threading

bot = telebot.TeleBot(TOKEN)
data = {}
data_from_user={}
user_price={}
chat_id=data_from_user.keys()

import cloudscraper
scraper = cloudscraper.create_scraper(
    browser={
        'browser': 'chrome',
        'platform': 'android',
        'desktop': False
    }
)

cookies = {
    '__Secure-ab-group': '46;',
    'ob_theme': 'DARK;',
    'ADDRESSBOOKBAR_WEB_CLARIFICATION': '1716216895;',
    'is_cookies_accepted': '1;',
    'xcid': 'af3e5f0ce15183783bfe1512b44f9fca;',
    'is_adult_confirmed': 'true;',
    'rfuid': '', }


def get_data_from_wb():
    chat_id=list(data_from_user)[0]
    try:
        html = scraper.get(
            f"https://search.wb.ru/exactmatch/ru/common/v5/search?ab_testing=false&appType=1&curr=rub&dest=-1257786&query={data_from_user[chat_id]['mark']}%{data_from_user[chat_id]['category']}%{data_from_user[chat_id]['articul']}&resultset=catalog&sort=popular&spp=30&suppressSpellcheck=false",
            cookies=cookies).text
        res = json.loads(html)

    except Exception as e:
        print(f'Сайт wb не рад такому запросу, ошибка{e}')

    wb_product_id = ''
    wb_price = ''
    wb_brand = ''
    wb_name = ''

    for i in res['data']['products']:
        if i['id'] == int(data_from_user[chat_id]['articul']):
            wb_product_id = i['id']
            wb_name = i['name']
            wb_brand = i['brand']
            wb_price = int(i['sizes'][0]['price']['product'] / 100)
        else:
            return None

    if wb_price=='':
        time.sleep(2)
        get_data_from_wb()
    else:
        data[chat_id] = {'chat_id': chat_id, 'wb_product_id': wb_product_id, 'wb_price': wb_price, 'wb_name': wb_name,
                 'wb_brand': wb_brand}

def get_price_from_wb():
    chat_id=list(data_from_user)[0]
    try:
        html = scraper.get(
            f"https://search.wb.ru/exactmatch/ru/common/v5/search?ab_testing=false&appType=1&curr=rub&dest=-1257786&query={data_from_user[chat_id]['mark']}%{data_from_user[chat_id]['category']}%{data_from_user[chat_id]['articul']}&resultset=catalog&sort=popular&spp=30&suppressSpellcheck=false",
            cookies=cookies).text
        res = json.loads(html)
    except Exception as e:
        print(f'Сайт wb не рад такому запросу, ошибка{e}')
    wb_price = 0

    for i in res['data']['products']:
        if i['id'] == int(data_from_user[chat_id]['articul']):
            wb_price = int(i['sizes'][0]['price']['product'] / 100)

    if wb_price != None:
        return wb_price
    else:
        time.sleep(2)
        get_data_from_wb()

def stop_bot():
    bot.stop_polling()


# Обработчик команды /stop
@bot.message_handler(commands=['stop'])
def handle_stop(message):
    bot.reply_to(message, "Бот останавливается...")
    threading.Thread(target=stop_bot).start()


@bot.message_handler(commands=['start'])
def start(message):
    bot.send_message(message.chat.id, f'Привет,\nнаш бот поможет тебе сэкономить на покупках Wildberries 😎')
    time.sleep(1)
    bot.send_message(message.chat.id, f'Для того чтобы начать:')
    time.sleep(1)
    bot.send_message(message.chat.id, f'введите марку товара, например\n<b>Nike</b>',parse_mode='HTML')
    bot.send_message(message.chat.id, f'введите категорию товара, например\n<b>Обувь</b>',parse_mode='HTML')
    bot.send_message(message.chat.id, f'введите артикул товара на сайте Wildberries, например\n<b>161110984</b>',parse_mode='HTML')
    bot.send_message(message.chat.id, f'Пример хорошего запроса:\n<b>adidas обувь 171612584</b> или\n<b>beko техника 119998055</b>\n<i>❗данные вводятся через пробел- это Важный момент</i>',parse_mode='HTML')
    bot.register_next_step_handler(message, get_data_from_user)


def get_data_from_user(message):
    reply=message.text
    print(reply)
    chat_id = message.chat.id
    print(chat_id)
    if len(reply.split())!=3:
        bot.send_message(chat_id,f'неправильный запрос')
        start(message)
    else:
        mark, category, articul = reply.strip().split(' ')
        data_from_user[chat_id]={'mark':mark,'category':category,'articul':articul}
        bot.send_message(chat_id,f'Обрабатываем ваш запрос... это может занять время 🚀️')
        get_data_from_wb()
        print(chat_id)
        try:
            bot.send_message(chat_id,
                         f"Нашли ваш товар, это:\nМодель-{data[chat_id]['wb_name']}\nБренд-{data[chat_id]['wb_brand']}\nАртикул-{data[chat_id]['wb_product_id']}\nЦена-{data[chat_id]['wb_price']}")
            bot.send_message(chat_id,
                             f'Проверяйте, если все верно\nвведите <b>Да</b>,\nхотите изменить товар?\nвведите<b> Замена</b>',
                             parse_mode='HTML')
            bot.register_next_step_handler(message, nextstep)
        except KeyError as e:
            print(f'ошибка {e}')
            bot.send_message(chat_id,f'Wildberries не спешит отвечать=) попробуем снова')
            time.sleep(2)
            get_data_from_user(message)


def nextstep(message):

    reply = message.text.lower()
    if reply == 'да':
        bot.send_message(message.chat.id,f'Сейчас ваш товар стоит: <b>{data[message.chat.id]["wb_price"]}₽</b>\nно мы знаем, что есть дни когда он стоит дешевле' ,parse_mode='HTML')
        bot.send_message(message.chat.id,f'Например Стирально-сушильная машина Beko с артикулом 119998055 на прошлой неделе стоила на <b>2215</b> рублей дешевле' ,parse_mode='HTML')
        time.sleep(1)
        bot.send_message(message.chat.id,f'бот может присылать уведомление- о любом снижении цены\nДля этого нажмите кнопку <b>Любое снижение цены</b>',parse_mode='HTML')
        bot.send_message(message.chat.id,f'Если вы хотите указать цену самостоятельно\n<b>отправьте боту вашу цену</b>, и далее нажмите\n<b>Укажу сумму самостоятельно</b>',parse_mode='HTML')
        time.sleep(1)

        markup = types.InlineKeyboardMarkup(row_width=1)
        btn_1 = types.InlineKeyboardButton('Любое снижение цены', callback_data='price_down')
        btn_2 = types.InlineKeyboardButton('Укажу сумму самостоятельно', callback_data='user_price_down')
        markup.add(btn_1, btn_2)
        bot.send_message(message.chat.id, f'Что вам подходит?', reply_markup=markup)
        bot.register_next_step_handler(message, get_user_price)

    else:
        start(message)


def get_user_price(message):
    chat_id=message.chat.id
    reply=message.text.lower()
    user_price[chat_id]=int(reply)

def check_price_down():
    wb_price=get_price_from_wb()
    chat_id = list(data_from_user)[0]
    if wb_price>0 and data[chat_id]['wb_price']>0:
        while True:
            bot.send_message(chat_id,f"цена wb {wb_price}, старая цена {data[chat_id]['wb_price']}")
            for i in data:
                if wb_price < data[i]['wb_price']:
                    bot.send_message(i, f"Успейте купить!\n Цена стала ниже,\nтеперь {wb_price}₽ вместо {data[i]['wb_price']}₽ ")
                    data[i]['wb_price']=wb_price
            time.sleep(20)
    else:
        check_price_down()

def check_user_price_down():
    chat_id = list(data_from_user)[0]
    user_price_compare=user_price[chat_id]
    wb_price=get_price_from_wb()
    if wb_price>0 and user_price_compare>0:
        while True:
            bot.send_message(chat_id, f"цена wb {wb_price}, цена юзера {user_price_compare}")
            if wb_price < user_price_compare:
                bot.send_message(chat_id, f"Успейте купить! Цена стала ниже,\nтеперь {wb_price}₽")
                break
            time.sleep(20)
    else:
        check_user_price_down()

thread = threading.Thread(target=check_price_down)
thread2 = threading.Thread(target=check_user_price_down)


@bot.callback_query_handler(func=lambda call: True)
def callback(call):

    if call.data=='price_down':
        bot.send_message(call.message.chat.id, f'Мы будем сравнивать цену. Бот пришлет сообщение, когда цена снизится')
        bot.send_message(call.message.chat.id,f'Сравниваю цену с ЦЕНОЙ Wildberries')
        thread.start()


    elif call.data=='user_price_down':
        chat_id = list(data_from_user)[0]
        user_price_compare = user_price[chat_id]
        bot.send_message(call.message.chat.id, f'Мы зафиксировали вашу цену {user_price_compare}₽. Бот пришлет сообщение,когда цена на WB станет меньше указанной')
        bot.send_message(call.message.chat.id, f'Сравниваю цену c ЦЕНОЙ юзера')
        thread2.start()



print('bot working')
if __name__=='__main__':
    bot.polling()

